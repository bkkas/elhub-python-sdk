...
from elhub_sdk.enums import SCHEME_AGENCY_IDENTIFIER, LIST_AGENCY_IDENTIFIER, DOCUMENT_TYPE_EBIX, BSR_IDS, ROLES, \
    QUERY_MARKET, DOCUMENT_TYPE_UN_CEFACT
...
def request_consumption(meter_identificator:str, third_party_gsn: str, start: datetime, end: datetime)->Optional[dict]:
    ...
    eh_request = factory.RequestDataFromElhub(
        ...
        ProcessEnergyContext={  # https://dok.elhub.no/ediel200utkast/general#General-Process
            'EnergyBusinessProcess': {'_value_1': BSR_IDS.METERING_VALUES.value, 'listAgencyIdentifier': LIST_AGENCY_IDENTIFIER.ELHUB.value},
            'EnergyBusinessProcessRole': {'_value_1': ROLES.THIRD_PARTY.value, 'listAgencyIdentifier': LIST_AGENCY_IDENTIFIER.UN_CEFACT.value},
...
def poll_consumption(third_party_gsn: str)->Optional[str]:
    ...
    request = factory.PollForData(
        Header={
            'Identification': uuid.uuid4(),
            'DocumentType': {'_value_1': DOCUMENT_TYPE_UN_CEFACT.QUERY.value, 'listAgencyIdentifier': LIST_AGENCY_IDENTIFIER.UN_CEFACT.value},
            'Creation': f'{datetime.utcnow().strftime(TIME_FORMAT)}',
            'PhysicalSenderEnergyParty': {
                'Identification': {'_value_1': third_party_gsn, 'schemeAgencyIdentifier': SCHEME_AGENCY_IDENTIFIER.GS1.value}},
            'JuridicalSenderEnergyParty': {
                'Identification': {'_value_1': third_party_gsn, 'schemeAgencyIdentifier': SCHEME_AGENCY_IDENTIFIER.GS1.value}},
            'JuridicalRecipientEnergyParty': {'Identification': {'_value_1': ELHUB_GSN, 'schemeAgencyIdentifier': SCHEME_AGENCY_IDENTIFIER.GS1.value}},
        },
        ProcessEnergyContext={
            'EnergyBusinessProcess': {'_value_1': 'POLL', 'listAgencyIdentifier': LIST_AGENCY_IDENTIFIER.ELHUB.value},
            'EnergyBusinessProcessRole': {'_value_1': 'DDQ', 'listAgencyIdentifier': LIST_AGENCY_IDENTIFIER.UN_CEFACT.value},
            'EnergyIndustryClassification': "23",
        },
        Payload={}
    )
